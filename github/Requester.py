...
class Requester:
    def __init__(self, id_or_token:str, base_url:str):
    ...
    def request_to_hub(self, headers:str, request_type:str, route:str, update_data:str, success_code:int=200):
        full_url:str = self._base_url + route

        res = self._separate_request_type(headers, request_type, full_url, update_data)
        succeed_in_receiving_response = self._check_status(res, success_code)

        if succeed_in_receiving_response: return res.json()
        else: return
